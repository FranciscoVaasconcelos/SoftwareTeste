...
uses
    ...
    uFormBase, Vcl.StdCtrls, Vcl.ExtCtrls, cxGraphics, cxControls,
    cxLookAndFeels, cxLookAndFeelPainters, cxContainer, cxEdit, cxTextEdit,
    cxCurrencyEdit, FireDAC.Comp.Client, Vcl.Mask, RxToolEdit;
...
type
    ...
    { Private declarations }
        ...
        function VerificarQuantidadeTanque(idTanque, idCombustivel: integer; QuantidadeEntrada: double): boolean;
    public
    { Public declarations }
    end;
...
begin
    ...
    if not(VerificarQuantidadeTanque(ResultComboSelecionado(cmbTanque), ResultComboSelecionado(cmbTipoComb), edtQuantEntrada.Value))then
    begin
        Erro('A quantidade de entrada não pode ser maoir que a quantidade de capacidade do tanque!');
        Abort;
    end;
    if Salvar then
    begin
...
function TfrmCadEntrada.VerificarQuantidadeTanque(idTanque,
  idCombustivel: integer; QuantidadeEntrada: double): boolean;
var
    qr: TFDQuery;
    Diferenca: double;
begin
    ...
    try
        qr.SQL.Add(DM.FDScript1.SQLScripts.Items[3].SQL.Text);
        qr.ParamByName('ID').AsInteger:= idTanque;
        qr.Open;
        Diferenca:= qr.FieldByName('Capacidade').AsFloat - qr.FieldByName('QuantAtualNoTanque').AsFloat;
        if QuantidadeEntrada > Diferenca then
        begin
            Result:= False;
        end
        else
